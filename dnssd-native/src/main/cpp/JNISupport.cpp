...
#endif
        jfieldID hasAutoCField = pEnv->GetStaticFieldID( cls, "hasAutoCallbacks", "Z");
        pEnv->SetStaticBooleanField( cls, hasAutoCField, hasAutoC);
    }
...
{
    return str != NULL ? pEnv->GetStringUTFChars( str, 0) : NULL;
}
...
{
    if ( str != NULL)
        pEnv->ReleaseStringUTFChars( str, buff);
}
...
// Create and initialize a new OpContext.
{
    ...
    if ( pContext != NULL)
    {
        jfieldID clientField = pEnv->GetFieldID( pEnv->GetObjectClass( owner),
                                                    "fListener", "Lcom/apple/dnssd/BaseListener;");

        pContext->JavaObj = pEnv->NewWeakGlobalRef( owner);    // must convert local ref to global to cache;
        pContext->ClientObj = pEnv->GetObjectField( owner, clientField);
        pContext->ClientObj = pEnv->NewWeakGlobalRef( pContext->ClientObj);    // must convert local ref to global to cache
        pContext->Callback = pEnv->GetMethodID(
                                                   pEnv->GetObjectClass( pContext->ClientObj),
...
{
    jclass cls = pEnv->GetObjectClass( target);
    jmethodID opFailed = pEnv->GetMethodID( cls, "operationFailed",
                                               "(Lcom/apple/dnssd/DNSSDService;I)V");

    pEnv->CallVoidMethod( target, opFailed, service, err);
}
...
jfieldID contextField = pEnv->GetFieldID( cls, "fNativeContext", "J");

if ( contextField != 0)
{
OpContext   *pContext = (OpContext*) (long) pEnv->GetLongField( pThis, contextField);
if ( pContext != NULL)
{
// MUST clear fNativeContext first, BEFORE calling DNSServiceRefDeallocate()
pEnv->SetLongField( pThis, contextField, 0);
if ( pContext->ServiceRef != NULL)
DNSServiceRefDeallocate( pContext->ServiceRef);

pEnv->DeleteWeakGlobalRef( pContext->JavaObj);
pEnv->DeleteWeakGlobalRef( pContext->ClientObj);
free( pContext);
}
}
}
...
{
pContext->Env->CallVoidMethod( pContext->ClientObj,
( flags & kDNSServiceFlagsAdd) != 0 ? pContext->Callback : pContext->Callback2,
pContext->JavaObj, flags, interfaceIndex,
pContext->Env->NewStringUTF(serviceName),
pContext->Env->NewStringUTF(regtype),
pContext->Env->NewStringUTF(replyDomain));
}
else
                                                                        ...
                                                                        jint flags, jint ifIndex, jstring regType, jstring domain)
{
    ...
    if ( pContext != NULL)
    {
        ...
        pContext->Callback2 = pEnv->GetMethodID(
                                                    pEnv->GetObjectClass( pContext->ClientObj),
        ...
        {
            pEnv->SetLongField( pThis, contextField, (long) pContext);
        }
...
txtCls = pContext->Env->FindClass("com/apple/dnssd/TXTRecord");
txtCtor = pContext->Env->GetMethodID(txtCls, "<init>", "([B)V");

if ( pContext->ClientObj != NULL && pContext->Callback != NULL && txtCtor != NULL &&
NULL != ( txtBytes = pContext->Env->NewByteArray(txtLen)))
{
if ( errorCode == kDNSServiceErr_NoError)
{
...
pBytes = pContext->Env->GetByteArrayElements(txtBytes, NULL);
memcpy( pBytes, txtRecord, txtLen);
pContext->Env->ReleaseByteArrayElements(txtBytes, pBytes, JNI_COMMIT);

// Construct txtObj with txtBytes
txtObj = pContext->Env->NewObject(txtCls, txtCtor, txtBytes);
pContext->Env->DeleteLocalRef(txtBytes);

pContext->Env->CallVoidMethod(pContext->ClientObj, pContext->Callback,
pContext->JavaObj, flags, interfaceIndex,
pContext->Env->NewStringUTF(fullname),
pContext->Env->NewStringUTF(hosttarget),
port, txtObj);
}
else
ReportError( pContext->Env, pContext->ClientObj, pContext->JavaObj, errorCode);
...
pContext->Env->NewStringUTF(serviceName),
pContext->Env->NewStringUTF(regType),
pContext->Env->NewStringUTF(domain));
}
else
ReportError( pContext->Env, pContext->ClientObj, pContext->JavaObj, errorCode);
}
                                                                             ...
                                                                             jstring domain, jstring host, jint port, jbyteArray txtRecord)
{
    ...
    if ( pContext != NULL)
    {
        ...
        pBytes = txtRecord ? pEnv->GetByteArrayElements( txtRecord, NULL) : NULL;
        numBytes = txtRecord ? pEnv->GetArrayLength( txtRecord) : 0;
        ...
        if ( pBytes != NULL)
            pEnv->ReleaseByteArrayElements( txtRecord, pBytes, 0);
                                                                         ...
                                                                         jint flags, jint rrType, jbyteArray rData, jint ttl, jobject destObj)
{
    jclass cls = pEnv->GetObjectClass( pThis);
    jfieldID contextField = pEnv->GetFieldID( cls, "fNativeContext", "J");
    jclass destCls = pEnv->GetObjectClass( destObj);
    jfieldID recField = pEnv->GetFieldID( destCls, "fRecord", "J");
    OpContext               *pContext = NULL;
    DNSServiceErrorType err = kDNSServiceErr_NoError;
    jbyte                   *pBytes;
    jsize numBytes;
    ...
    if ( contextField != 0)
        pContext = (OpContext*) (long) pEnv->GetLongField( pThis, contextField);
    if ( pContext == NULL || pContext->ServiceRef == NULL)
        return kDNSServiceErr_BadParam;

    pBytes = pEnv->GetByteArrayElements( rData, NULL);
    numBytes = pEnv->GetArrayLength( rData);

    err = DNSServiceAddRecord( pContext->ServiceRef, &recRef, flags, rrType, numBytes, pBytes, ttl);
    if ( err == kDNSServiceErr_NoError)
    {
        pEnv->SetLongField( destObj, recField, (long) recRef);
    }

    if ( pBytes != NULL)
        pEnv->ReleaseByteArrayElements( rData, pBytes, 0);
...
{
    jclass cls = pEnv->GetObjectClass( pThis);
    jfieldID ownerField = pEnv->GetFieldID( cls, "fOwner", "Lcom/apple/dnssd/AppleService;");
    jfieldID recField = pEnv->GetFieldID( cls, "fRecord", "J");
    OpContext               *pContext = NULL;
    DNSServiceErrorType err = kDNSServiceErr_NoError;
    jbyte                   *pBytes;
    jsize numBytes;
    ...
    if ( ownerField != 0)
    {
        jobject ownerObj = pEnv->GetObjectField( pThis, ownerField);
        jclass ownerClass = pEnv->GetObjectClass( ownerObj);
        jfieldID contextField = pEnv->GetFieldID( ownerClass, "fNativeContext", "J");
        if ( contextField != 0)
            pContext = (OpContext*) (long) pEnv->GetLongField( ownerObj, contextField);
    }
    if ( recField != 0)
        recRef = (DNSRecordRef) (long) pEnv->GetLongField( pThis, recField);
    if ( pContext == NULL || pContext->ServiceRef == NULL)
        return kDNSServiceErr_BadParam;
...
pContext->Env->DeleteWeakGlobalRef(regEnvelope->RecordObj);
free( regEnvelope);
                                                                                 ...
                                                                                 jbyteArray rData, jint ttl, jobject destObj)
{
    ...
    regEnvelope = (RecordRegistrationRef *) calloc( 1, sizeof *regEnvelope);
    if ( regEnvelope == NULL)
        return kDNSServiceErr_NoMemory;
    regEnvelope->Context = pContext;
    regEnvelope->RecordObj = pEnv->NewWeakGlobalRef( destObj); // must convert local ref to global to cache
    ...
    else
    {
        if ( regEnvelope->RecordObj != NULL)
            pEnv->DeleteWeakGlobalRef( regEnvelope->RecordObj);
        free( regEnvelope);
    }
...
if ( pContext->ClientObj != NULL && pContext->Callback != NULL &&
NULL != ( rDataObj = pContext->Env->NewByteArray(rdlen)))
{
if ( errorCode == kDNSServiceErr_NoError)
{
// Initialize rDataObj with contents of rdata
pBytes = pContext->Env->GetByteArrayElements(rDataObj, NULL);
memcpy( pBytes, rdata, rdlen);
pContext->Env->ReleaseByteArrayElements(rDataObj, pBytes, JNI_COMMIT);
                                                                      ...
                                                                      jstring serviceName, jstring regtype, jstring domain, jobjectArray pOut)
{
    ...
    if ( err == kDNSServiceErr_NoError)
    {
        ...
        pEnv->SetObjectArrayElement( pOut, 0, pEnv->NewStringUTF( buff));
    }
