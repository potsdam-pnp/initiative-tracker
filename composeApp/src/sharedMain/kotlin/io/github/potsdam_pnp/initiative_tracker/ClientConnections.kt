...
class ClientConnections(
  ...
  suspend fun run(scope: CoroutineScope) {
    ...
    val f =
      { connections: Map<ClientIdentifier, ConnectionState>, services: Map<String, ServiceInfo> ->
        services
          .mapValues { entry -> entry.value to connections[entry.value.clientId] }
          .filterValues {
            it.first.clientId != null &&
              it.second?.let { !it.clientConnected && !it.serverConnected } != false
          }
      }
