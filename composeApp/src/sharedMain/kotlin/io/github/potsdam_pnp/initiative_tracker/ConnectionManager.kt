...
data class ConnectionInformation(val port: Int, val hosts: List<String>)
...
abstract class ConnectionManager() {
  ...
  val connectionStates: MutableStateFlow<Map<ClientIdentifier, ConnectionState>> =
    ...
    serviceInfoState.collect {
      ...
      if (newClients.isNotEmpty()) {
        serviceInfoState.update { info ->
          info +
            newClients.mapNotNull { client ->
              info[client.first]?.let { client.first to it.copy(clientId = client.second) }
            }
        }
        delay(4000)
      }
...
fun toServerStatus(
  connectionStates: Map<ClientIdentifier, ConnectionState>,
  serviceInfoStates: Map<String, ServiceInfo>,
  name: String?,
): ServerStatus {
  ...
  return ServerStatus(
    ...
    joinLinks =
      name
        ?.let { serviceInfoStates[it] }
        ?.connectionInformation
        ?.let { it.hosts.map { JoinLink(it) } } ?: emptyList(),
    discoveredClients =
      serviceInfoStates.map {
